function [PRisk, PRoR, PWts] = frontconNew(ERet, ECov, NPts, RTarget, ACLUBounds, ACGroups, ACGLUBounds)
...
%   $Revision: 1.15.2.4 $   $ Date: 1998/01/30 13:45:34 $
...
if (nargin < 2)
     error('finance:frontcon:missingInputs','You must enter ExpReturn and ExpCovariance.');
end
...
if any( size(ECov)~=NASSETS )
    error('finance:frontcon:nonSquareCovMatrix','The covariance matrix must be NxN, where N = number of assets');
end
...
if (NPtsEntered ==1) & (RTargetEntered == 1)
  if (length(RTarget(:)) ~= NPts )
    error('finance:frontcon:needEmptyNumPorts','When entering PortReturn, enter NumPorts as an empty matrix')
  end
end
...
if nargout == 0
   portoptNew(ERet, ECov, NPts, RTarget, ConSet);
else
   [PRisk, PRoR, PWts] = portoptNew(ERet, ECov, NPts, RTarget, ConSet);
end
