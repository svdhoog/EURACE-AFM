...
%  Help on advanced usage is obtained by typing laprint({'AdvancedHelp'}).
%
...
vers=eval(vers(1:3));
if vers < 5.0
  error('Sorry. Matlab 5.0 or above is required.')
end
...
uicontrol('Parent',hf,'style','frame','Units','points',...
	  'Position',[0 0 31*fsize figheight])
...
set(h,'Callback','laprint({''filename''});')
...
set(h,'Callback','laprint({''commenttext''});')
...
showtext({['This is LaPrint, Version ' laprintident ...
           ', by Arno Linnemann.'],...
	   'To get started, press ''Help'' below.', txt })
showsizes;
%---------------------------------
% save, quit, help
%---------------------------------
...
set(h,'HorizontalAlignment','center')
set(h,'String','Save')
set(h,'Callback','laprint({''save''});')

%---------------------------------
% options uimenue
%---------------------------------

hm=uimenu('label','LaPrint');
...
LAPRINTHAN.loose=uimenu(hm,...
       'label','print loose',...
       'callback','laprint({''loose''})',...
       'checked','off');

LAPRINTHAN.nofigcopy=uimenu(hm,...
       'label','figure copy',...
       'callback','laprint({''nofigcopy''})',...
       'checked','on');

LAPRINTHAN.nohead=uimenu(hm,...
       'label','file head',...
       'callback','laprint({''nohead''})',...
       'checked','on');

LAPRINTHAN.noscalefonts=uimenu(hm,...
       'label','scale fonts',...
       'callback','laprint({''noscalefonts''})',...
       'checked','on');
...
if isa(figno,'cell')
  %%% figno %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  if strcmp(figno{1},'figno')
    global LAPRINTOPT
    global LAPRINTHAN
    LAPRINTOPT.figno=eval(get(LAPRINTHAN.figno,'string'));
    figure(LAPRINTOPT.figno)
    figure(131)
    txt=[ 'Pushing ''save'' will save the contents of Figure No. '...
	  int2str(LAPRINTOPT.figno) '.' ];
    showtext({txt});
  %%% filename %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  elseif strcmp(figno{1},'filename')
    global LAPRINTOPT
    global LAPRINTHAN
  ...
  elseif strcmp(figno{1},'factor')
    ...
    showtext({txt1,txt2});
    showsizes;
  elseif strcmp(figno{1},'commenttext')
    global LAPRINTOPT
    global LAPRINTHAN
    ...
    showtext({txt});
  %%% options %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  elseif strcmp(figno{1},'asonscreen')
    global LAPRINTOPT
    global LAPRINTHAN
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'keepticklabels')
    global LAPRINTOPT
    global LAPRINTHAN
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'mathticklabels')
    global LAPRINTOPT
    global LAPRINTHAN
    txt1=[ 'With this option set to ''on'', the tick labels '...
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'keepfontprops')
    global LAPRINTOPT
    global LAPRINTHAN
  ...
  elseif strcmp(figno{1},'loose')
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'extrapicture')
    global LAPRINTOPT
    global LAPRINTHAN
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'nofigcopy')
    global LAPRINTOPT
    global LAPRINTHAN
  ...
  elseif strcmp(figno{1},'nohead')
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'noscalefonts')
    global LAPRINTOPT
    global LAPRINTHAN
  ...
  elseif strcmp(figno{1},'caption')
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'viewfile')
    global LAPRINTOPT
    global LAPRINTHAN
    ...
    showtext({txt1, txt2});
  elseif strcmp(figno{1},'defaults')
    global LAPRINTOPT
    global LAPRINTHAN
    LAPRINTOPT.asonscreen=0;
    LAPRINTOPT.keepticklabels=0;
  ...
  elseif strcmp(figno{1},'help')
    ...
    showtext(txt);
  %%% quit %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  elseif strcmp(figno{1},'quit')
    LAPRINTHAN=[];
    LAPRINTOPT=[];
    close(131)
  %%% Advanced Help %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
  elseif strcmp(lower(figno{1}),'advancedhelp')
    disp(' ')
    disp('Advanced usage: laprint(figno,filename,opt1,opt2,..)')
    disp('where ')
